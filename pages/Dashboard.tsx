    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="xl:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, idx) => (
            <div key={idx} className="bg-white p-5 rounded border-b-4 border-slate-200 shadow-sm flex flex-col justify-between hover:border-slate-400 transition-all">
          ...
          {/* Progress Chart Integrated into stats row or below */}
          <div className="sm:col-span-2 lg:col-span-4 bg-white p-6 rounded border-b-4 border-slate-200 shadow-sm h-80">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-industrial font-bold text-slate-700 flex items-center gap-2">
              ...
              <div className="flex gap-4 text-[10px] font-bold uppercase tracking-tighter">
                ...
                <div className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#3C3C3C]"></span> Workforce</div>
              </div>
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Tooltip contentStyle={{backgroundColor: '#1A1A1A', border: 'none', borderRadius: '4px', color: '#fff', fontSize: '12px'}} />
                  <Area type="monotone" dataKey="progress" stroke="#F5C518" strokeWidth={3} fillOpacity={1} fill="url(#colorProg)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-[#1A1A1A] rounded overflow-hidden flex flex-col shadow-xl border-b-4 border-[#F5C518]">
          <div className="p-4 bg-[#000] border-b border-white/5 flex items-center justify-between">
            <span className="font-industrial text-[#F5C518] text-sm font-bold flex items-center gap-2">
              <i className="fa-solid fa-brain"></i> AI Site Intel
            </span>
            <span className="text-[9px] bg-white/10 text-white/40 px-2 py-0.5 rounded font-bold uppercase">v2.5 Pro</span>
          </div>
          ...
          <div className="flex-1 p-6 flex flex-col">
            {loadingAI ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <p className="text-slate-400 font-industrial text-sm animate-pulse">Scanning site telemetry...</p>
              </div>
            ) : prediction ? (
              <div className="space-y-6">
                <div>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Predicted Delay</p>
                ...
                <div className="p-4 bg-white/5 rounded border border-white/10 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded border border-slate-200 overflow-hidden shadow-sm">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
            <h3 className="font-industrial font-bold text-slate-800">Critical Tasks</h3>
            <span className="bg-red-100 text-red-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase">Immediate Action</span>
          </div>
        ...
        <div className="bg-white rounded border border-slate-200 overflow-hidden shadow-sm">
           <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
            <h3 className="font-industrial font-bold text-slate-800">Supply Alerts</h3>
            <span className="bg-amber-100 text-amber-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase">Inventory Audit</span>
          </div>
          <div className="p-4 space-y-4">
             {MOCK_MATERIALS.filter(m => m.quantity < m.reorderLevel).map(m => (
               <div key={m.id} className="flex items-center gap-4 bg-amber-50/50 p-3 rounded border border-amber-100">
                  <div className="w-10 h-10 bg-white rounded flex items-center justify-center text-amber-600 shadow-sm border border-amber-100">
                    <i className="fa-solid fa-truck-loading"></i>
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between">
                      <span className="text-sm font-bold text-slate-800">{m.itemName}</span>
                      <span className="text-xs font-bold text-red-600">{m.quantity} {m.unit}</span>
                    </div>
                    <div className="text-[10px] text-slate-500 font-bold uppercase mt-1">Reorder Level: {m.reorderLevel} {m.unit}</div>
                  </div>
                  <button className="px-3 py-1 bg-[#F5C518] text-[#000] text-[10px] font-bold rounded uppercase shadow-[0_2px_0_#C49D13]">Order Now</button>
               </div>
             ))}
          </div>
        </div>
